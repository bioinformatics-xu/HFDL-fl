...
CTRP_response = pd.read_csv("./data/commonWaterfall/CTRP_response.csv",index_col=0)
CTRP_response_class = pd.read_csv("./data/commonWaterfall/CTRP_response_class_common_waterfall.csv",index_col=0)
GDSC_response = pd.read_csv("./data/commonWaterfall/GDSC2_response.csv",index_col=0)
GDSC_response_class = pd.read_csv("./data/commonWaterfall/GDSC2_response_class_common_waterfall.csv",index_col=0)

# 'AZD7762', 'PLX-4720', 'olaparib', 'dasatinib', 'docetaxel:tanespimycin (2:1 mol/mol)','linsitinib','fluorouracil'
CTRP_response_common = CTRP_response.loc[:,['AZD7762', 'PLX-4720', 'olaparib', 'linsitinib','fluorouracil']]
CTRP_response_common.columns = ['AZD7762', 'PLX4720', 'Olaparib', 'Linsitinib','Fluorouracil']

GDSC_response_common = GDSC_response.loc[:,['AZD7762_1022', 'PLX-4720_1036', 'Olaparib_1017', 'Linsitinib_1510', '5-Fluorouracil_1073']]
GDSC_response_common.columns = ['AZD7762', 'PLX4720', 'Olaparib', 'Linsitinib','Fluorouracil']
drug_list = GDSC_response_common.columns

#plot the response distribution in GDSC and CTRP
pp = PdfPages('./figures_FL_DRP/response_distribution.pdf')
fig,axes=plt.subplots(2,5)

for gi in range(len(drug_list)):
    d = np.array(np.log(GDSC_response_common[drug_list[gi]]).tolist())
...
def plot_sample_number(class_statistic, y_label):
    ...
    # plt.ylim(0, 1)
    plt.bar(x1, y1, width=width / 2, label='resistant')
    plt.bar(x2, y2, width=width / 2, label='intermediate')
    plt.bar(x3, y3, width=width / 2, label='sensitive')
...
# plt.subplot(133)
plt.tight_layout()
plt.savefig('./figures_FL_DRP/sample_number.pdf', bbox_inches='tight') # 保存成PDF放大后不失真（默认保存在了当前文件夹下）
plt.show()
