...
class Focal_Loss(nn.Module):
    def __init__(self, weight, gamma=3):
    ...
    if len(class_counts) == 3:
        ...
        weight_loss = torch.tensor(
                [(1 / c0) / (1 / c0 + 1 / c1 + 1 / c2), (1 / c1) / (1 / c0 + 1 / c1 + 1 / c2),
                 (1 / c2) / (1 / c0 + 1 / c1 + 1 / c2)])
        criterion = Focal_Loss(weight_loss).to(device)
...
def train_net_fedprox(net_id, net, global_net, train_dataloader, test_dataloader, epochs, lr, args_optimizer, mu, device="cpu"):
    ...
    target_list = train_dataloader.dataset.targets.tolist()
    class_counts = Counter(target_list)
    if len(class_counts) == 3:
        c0 = class_counts[0] / sum(target_list)
        c1 = class_counts[1] / sum(target_list)
...
def local_train_net(nets, selected, args, net_dataidx_map, test_dl = None, device="cpu", round = None):
    ...
    for net_id, net in nets.items():
        ...
        if net.output_dim==2:
            ...
            if os.path.exists((args.datadir + '/results/results_local' + '.csv')):
                list_result = [str(args.dataset), args.alg, args.partition, args.n_parties, round, net_id, len(test_dl),
                               testacc, testauc]
                write_csv('results_local', list_result)
            else:
                create_csv('results_local',
                           ['drug_name', 'algorithm', 'partition', 'n_parties', 'round', 'net_id', 'test_num',
    ...
    if net.output_dim==2:
        list_result = [str(args.dataset), args.alg, args.partition, args.n_parties, round, "avg test acc", len(test_dl),
                       avg_acc]
        write_csv('results_local', list_result)
    elif net.output_dim==3:
        list_result = [str(args.dataset), args.alg, args.partition, args.n_parties, round, "avg test acc", len(test_dl),
...
def local_train_net_fedprox(nets, selected, global_model, args, net_dataidx_map, test_dl = None, device="cpu", round=None):
    avg_acc = 0.0
    avg_macro_F1 = 0.0
    avg_weighted_F1 = 0.0
    ...
    for net_id, net in nets.items():
        ...
        if net.output_dim == 2:
            trainacc, testacc, testauc = train_net_fedprox(net_id, net, global_model, train_dl_local, test_dl, n_epoch,
                                                           args.lr, args.optimizer, args.mu, device=device)
            ...
            trainacc, testacc, macro_F1, weighted_F1 = train_net_fedprox(net_id, net, global_model, train_dl_local, test_dl, n_epoch, args.lr,
...
def local_train_net_moon(nets, selected, args, net_dataidx_map, test_dl=None, global_model = None, prev_model_pool = None, round=None, device="cpu"):
    ...
    for net_id, net in nets.items():
        ...
        trainacc, testacc = train_net_moon(net_id, net, global_model, prev_models, train_dl_local, test_dl, n_epoch, args.lr,
                                              args.optimizer, args.mu, args.temperature, round, device=device)
        logger.info("net %d final test acc %f" % (net_id, testacc))
        avg_acc += testacc
...
if __name__ == '__main__':
    ...
    if args.alg == 'fedavg':
        ...
        for round in range(args.comm_round):
            ...
            if n_classes==2:
                ...
                if os.path.exists((args.datadir + '/results/results_global' + '.csv')):
                    list_result = [str(args.dataset), args.alg, args.partition, args.n_parties, round, train_acc,
                                   test_acc, test_auc]
                    write_csv('results_global', list_result)
                else:
                    create_csv('results_global',
                               ['drug_name', 'algorithm', 'partition', 'n_parties', 'round', 'Global_train_acc',
                                'Global_test_acc', 'Global_test_auc'])
                    list_result = [str(args.dataset), args.alg, args.partition, args.n_parties, round, train_acc,
                                   test_acc, test_auc]
                    write_csv('results_global', list_result)
                ...
                if os.path.exists((args.datadir + '/results/results_global' + '.csv')):
                    ...
                    create_csv('results_global',
                               ['drug_name', 'algorithm', 'partition', 'n_parties', 'round', 'Global_train_acc',
                                'Global_test_acc', "macro_F1", "weighted_F1"])
                    list_result = [str(args.dataset), args.alg, args.partition, args.n_parties, round, train_acc,
                                   test_acc, macro_F1, weighted_F1]
                    write_csv('results_global', list_result)
                if args.datadir == "./data/commonWaterfall":
                    test_acc_client0_d0, conf_matrix_client0_d0, macro_F1_client0_d0, weighted_F1_client0_d0 = compute_accuracy(nets[0], test_data0_dl,
                                                                                                get_confusion_matrix=True,
                                                                                                device=device,
                                                                                                auc=False)
                    test_acc_client0_d1, conf_matrix_client0_d1, macro_F1_client0_d1, weighted_F1_client0_d1 = compute_accuracy(nets[0], test_data1_dl,
                                                                                                get_confusion_matrix=True,
                                                                                                device=device,
                                                                                                auc=False)
                    test_acc_client1_d0, conf_matrix_client1_d0, macro_F1_client1_d0, weighted_F1_client1_d0 = compute_accuracy(nets[1], test_data0_dl,
                                                                                                get_confusion_matrix=True,
                                                                                                device=device,
                                                                                                auc=False)
                    test_acc_client1_d1, conf_matrix_client1_d1, macro_F1_client1_d1, weighted_F1_client1_d1 = compute_accuracy(nets[1], test_data1_dl,
                                                                                                get_confusion_matrix=True,
                                                                                                device=device,
                                                                                                auc=False)
                    ...
                    if os.path.exists((args.datadir + '/results/results_clientdata' + '.csv')):
                        list_result = [str(args.dataset), args.alg, args.partition, args.n_parties, round, 'client0_d0',
                                       test_acc_client0_d0, macro_F1_client0_d0, weighted_F1_client0_d0]
                        write_csv('results_clientdata', list_result)
                        list_result = [str(args.dataset), args.alg, args.partition, args.n_parties, round, 'client0_d1',
                                       test_acc_client0_d1, macro_F1_client0_d1, weighted_F1_client0_d1]
                        write_csv('results_clientdata', list_result)
                        list_result = [str(args.dataset), args.alg, args.partition, args.n_parties, round, 'client1_d0',
                                       test_acc_client1_d0, macro_F1_client1_d0, weighted_F1_client1_d0]
                        write_csv('results_clientdata', list_result)
                        list_result = [str(args.dataset), args.alg, args.partition, args.n_parties, round, 'client1_d1',
                                       test_acc_client1_d1, macro_F1_client1_d1, weighted_F1_client1_d1]
                        write_csv('results_clientdata', list_result)
                        list_result = [str(args.dataset), args.alg, args.partition, args.n_parties, round, 'global_d0',
                                       test_acc_global_d0, macro_F1_global_d0, weighted_F1_global_d0]
                        write_csv('results_clientdata', list_result)
                        list_result = [str(args.dataset), args.alg, args.partition, args.n_parties, round, 'global_d1',
                                       test_acc_global_d1, macro_F1_global_d1, weighted_F1_global_d1]
                        write_csv('results_clientdata', list_result)

                    else:
                        create_csv('results_clientdata',
                                   ['drug_name', 'algorithm', 'partition', 'n_parties', 'round', 'type',
                                    'test_acc', "macro_F1", "weighted_F1"])
                        list_result = [str(args.dataset), args.alg, args.partition, args.n_parties, round, 'client0_d0',
                                       test_acc_client0_d0, macro_F1_client0_d0, weighted_F1_client0_d0]
                        write_csv('results_clientdata', list_result)
        ...
        for round in range(args.comm_round):
            ...
            local_train_net_fedprox(nets, selected, global_model, args, net_dataidx_map, test_dl = test_dl_global, device=device, round=round)
            global_model.to('cpu')
    ...
    if os.path.exists((args.datadir + '/results/traindata_cls_count' + '.csv')):
        list_result = [str(args.dataset), args.alg, args.partition, args.n_parties, traindata_cls_counts]
        write_csv('traindata_cls_count', list_result)
    else:
        create_csv('traindata_cls_count',
