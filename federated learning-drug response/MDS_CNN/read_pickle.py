...
Results_Points[:, 0] = Y_Val;
Results_Points[:, 1] = Y_Test.reshape(len(Y_Test))
...
pca_xy_cell[:, 0] = np.round(
    (pca_xy_cell[:, 0] - min(pca_xy_cell[:, 0])) * (nn - 1) / (max(pca_xy_cell[:, 0]) - min(pca_xy_cell[:, 0])))
pca_xy_cell[:, 1] = np.round(
    (pca_xy_cell[:, 1] - min(pca_xy_cell[:, 1])) * (nn - 1) / (max(pca_xy_cell[:, 1]) - min(pca_xy_cell[:, 1])))
...
# Normalizing the PCA coordinates to fit in a [0,n] dimensional 2D image
pca_xy_Drug[:, 0] = np.round(
    (pca_xy_Drug[:, 0] - min(pca_xy_Drug[:, 0])) * (nn - 1) / (max(pca_xy_Drug[:, 0]) - min(pca_xy_Drug[:, 0])))
pca_xy_Drug[:, 1] = np.round(
    (pca_xy_Drug[:, 1] - min(pca_xy_Drug[:, 1])) * (nn - 1) / (max(pca_xy_Drug[:, 1]) - min(pca_xy_Drug[:, 1])))
...
X_Drug_Val_REFINED = REFINED_Im_Gen(X_Drug_Vl, nn, map_in_int_drug, gene_names_drug, coords_drug)
...
for Model_i in range(len(Model_Names)):
    if Model_i == 0:
        ...
        Width1 = int(math.sqrt(Shape1[1]));
        Height1 = int(math.sqrt(Shape1[1]))

        Shape2 = X_Cell_Train_Rand.shape
        Width2 = int(math.sqrt(Shape2[1]));
        Height2 = int(math.sqrt(Shape2[1]))
        # Reshaping the data
    ...
    CNN_History = CNNmodel.fit([IM_Training, IM_Training2], Y_Train, batch_size=128, epochs=50, verbose=0,
                               validation_data=([IM_Valid, IM_Valid2], Y_Val),
                               callbacks=[ES])  # callbacks = callbacks_list)
    ...
    Results_Models[0, Model_i] = CNN_NRMSE;
    Results_Models[1, Model_i] = CNN_PCC;
    Results_Models[2, Model_i] = CNN_R2
    Results_Models[3, Model_i] = CNN_MAE;
    Results_Models[4, Model_i] = CNN_Dist_Corr

    Y_Pred_Val_CNN = Y_Pred_Val_CNN.reshape(len(Y_Pred_Val_CNN))
    Y_Pred_CNN = Y_Pred_CNN.reshape(len(Y_Pred_CNN))
    Results_Points[:, 2 + 2 * Model_i] = Y_Pred_Val_CNN;
    Results_Points[:, 3 + 2 * Model_i] = Y_Pred_CNN

Results = pd.DataFrame(data=Results_Models, columns=['CNN Random', 'CNN PCA', 'CNN REFINED']
                       , index=['NRMSE', 'PCC', 'R2', 'MAE', 'Dist Corr'])
PD_Results_Points = pd.DataFrame(data=Results_Points,
                                 columns=['Y_Val', 'Y_Test', 'Y_Val_Rand', 'Y_Test_Rand', 'Y_Val_PCA', 'Y_Test_PCA',
                                          'Y_Val_REFINED', 'Y_Test_REFINED'])
