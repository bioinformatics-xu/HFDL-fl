...
for (j in 1:ncol(GDSC2_response)) {
  ...
  if(length(which(GDSC2_response[,j] > median_j*threshold_GDSC)) < floor(length(GDSC2_response_j)*0.05)){
    GDSC2_response_class[order(x=GDSC2_response_j,decreasing=TRUE)[1:floor(length(GDSC2_response_j)*0.05)],j] <- 0
  }
  if(length(which(GDSC2_response[,j] < median_j/threshold_GDSC)) < floor(length(GDSC2_response_j)*0.05)){
    GDSC2_response_class[order(x=GDSC2_response_j,decreasing=FALSE)[1:floor(length(GDSC2_response_j)*0.05)],j] <- 2
  }
}
...
for (j in 1:ncol(CTRP_response)) {
  ...
  if(length(which(CTRP_response[,j] > median_j*threshold_CTRP))<floor(length(CTRP_response_j)*0.05)){
    CTRP_response_class[order(x=CTRP_response_j,decreasing=TRUE)[1:floor(length(CTRP_response_j)*0.05)],j] <- 0
  }
  if(length(which(CTRP_response[,j] < median_j/threshold_CTRP))<floor(length(CTRP_response_j)*0.05)){
    CTRP_response_class[order(x=CTRP_response_j,decreasing=FALSE)[1:floor(length(CTRP_response_j)*0.05)],j] <- 2
  }
